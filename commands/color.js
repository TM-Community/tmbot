...
module.exports = {
  ...
  async execute({ interaction, data, locale }) {
    ...
    let newHex = hexcolor.startsWith("#") ? hexcolor : `#${hexcolor}`;
    if(!isHexColor(newHex)) return interaction.reply({ content: i18n.get("color.invalidHexColor", locale) })
    ...
    const buffer = Buffer.from(`<svg width="400" height="140">
        ...
        <rect width="5" height="140" fill="${newHex}"/>
        <path d="${icon}" transform="translate(29 42)" fill="${newHex}"/>
        <path d="${icon}" transform="translate(29 110)" fill="${newHex}"/>
        <text transform="translate(50 42)" fill="${newHex}" font-size="20" font-family="SegoeUI, Segoe UI"><tspan x="0" y="0">${interaction.user.tag}</tspan></text>
        <text transform="translate(50 110)" fill="${newHex}" font-size="20" font-family="SegoeUI, Segoe UI"><tspan x="0" y="0">${interaction.user.tag}</tspan></text>
    </svg>`)
    let bufferEdited = await sharp(buffer).png().toBuffer();

    const attachment = new MessageAttachment(bufferEdited, 'ffff.png'); 
